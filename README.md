...
```text
formula-insights-api merge request
        ↓
GitLab CI tests and builds an image
        ↓
...
- Every production-facing change is made through a merge request.
...
After a merge, verify that Argo CD reports the application as `Synced` and `Healthy`. The deployed version and relevant dashboard links should be included in the merge request.

## Related documents

See ADR-003 in `platform-docs` for the GitOps decision, and use `platform-docs/runbooks` for rollback and incident procedures.
